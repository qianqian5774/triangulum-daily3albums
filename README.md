...
This repo builds **once per day**. The site itself switches between today’s slots **at runtime** (no redeploy needed at unlock times).
...
2. Build the daily artifacts (generator writes runtime JSON into `_build/public/data`):
...
> Note: Local testing should only serve `_build/public`. Do not serve `ui/public/data` (or `ui/dist/data`) for runtime JSON.
...
- Append a query parameter to the site URL:

...
def test_tracked_product_surfaces_use_current_name_and_schedule():
    ...
    art_direction = (ROOT / "docs" / "art-direction.md").read_text(encoding="utf-8")
    share_template = (
    ...
    assert "TRIANGULUM DAILY" in art_direction
    assert "TRIANGULUM DAILY 3 ALBUMS" not in art_direction

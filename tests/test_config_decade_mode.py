...
def test_load_config_defaults_decade_mode_off_and_ignores_legacy_keys(tmp_path: Path):
    ...
    assert cfg.dedupe_same_rg_days == 7
    assert cfg.dedupe_same_artist_days == 7


def test_load_config_reads_active_history_cooldowns(tmp_path: Path):
    (tmp_path / "config").mkdir(parents=True, exist_ok=True)
    (tmp_path / "config" / "endpoint_policies.yaml").write_text("{}\n", encoding="utf-8")
    (tmp_path / "config" / "config.yaml").write_text(
        """
        timezone: Asia/Shanghai
        history:
          dedupe_same_rg_days: 9
          dedupe_same_artist_days: 5
        """,
        encoding="utf-8",
    )

    cfg = load_config(tmp_path)
    assert cfg.dedupe_same_rg_days == 9
    assert cfg.dedupe_same_artist_days == 5

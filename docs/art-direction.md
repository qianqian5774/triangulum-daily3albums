# Triangulum Daily Art Direction
...
正式产品名是 **Triangulum Daily**。当前页面发布结构统一理解为：
...
后续文案、分享卡和视觉结构统一使用 Triangulum Daily 与 daily 9 albums / 3 unlock windows / 3 albums each，不再使用历史旧名称。
...
- `signal-accent #A8C700`：夜间主识别色，像信号灯、入口灯、Rave flyer 上的酸性印刷色。
...
| `signal-accent` | `#A8C700` | 夜间主识别、当前状态、BJT、焦点、信号灯 |
...
- 08:00 / 12:30 / 16:00 可以被视觉化为三段 gate（入口）或 signal windows（信号窗口）。
...
- 08:00 / 12:30 / 16:00 应像三道入口、三段信号窗、三张档案抽屉。
...
第一版不要复杂。目标是按已解锁专辑生成可下载图片，不专门适配小红书、公众号、Instagram 或任何平台。
...
- 按三个时段组织，而不是随便九宫格堆专辑。
- 信息短，封面清楚。
- 支持 08:00 / 12:30 / 16:00 三个已解锁版本。
- 支持白昼 / 夜间两套主题，默认跟随当前网页主题。
- 支持中英双语，默认跟随当前 UI 语言。
...
当只解锁 08:00 时，分享卡只渲染 08:00 三张；当解锁到 12:30 时，可渲染 08:00 或 08:00+12:30；当解锁到 16:00 时，可渲染完整三段九张。
...
它应该像一个地下酸性唱片信号系统：黑底、酸绿、Y2K 故障、低清数字纹理、三段时窗、反信息流、反推荐算法。它每天按北京时间 08:00 / 12:30 / 16:00 释放九张专辑，让用户重新以 album as a unit（专辑为单位）听音乐。

...
12. Build UI。
13. Restore published archive seed 到 `.state/pages-history-seed/data`，并使用 `--max-days 7`。
...
18. Maintain `.state/cache.sqlite`。
19. Upload Pages artifact from `_build/public`。
20. 失败时向 GitHub issue 报告。
...
10. 写入 `today.json`、archive JSON 和 `index.json`。
